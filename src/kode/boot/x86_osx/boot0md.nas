...
exclude print_hex - no space for it
...
kBoot0Stack			EQU  0xFFF0			;
boot0 stack pointer
kBoot0LoadAddr		EQU  0x7C00			;
boot0 load address
kBoot0RelocAddr		EQU  0xE000			;
boot0 relocated address

kMBRBuffer			EQU  0x1000			;
MBR buffer address
kLBA1Buffer			EQU  0x1200			;
LBA1 - GPT Partition Table Header buffer address
kGPTABuffer			EQU  0x1400			;
GUID Partition Entry Array buffer address

kPartTableOffset	EQU  0x1be
kMBRPartTable		EQU  kMBRBuffer + kPartTableOffset

kSectorBytes		EQU  512			;
sector size in bytes
kBootSignature		EQU  0xAA55			;
boot sector signature
kHFSPSignature		EQU  'H+'			;
HFS + volume signature
kHFSPCaseSignature	EQU  'HX'			;

HFS + volume case-sensitive signature
kFAT32BootCodeOffset EQU  0x5a			;
offset of boot code in FAT32 boot sector
kBoot1FAT32Magic	EQU  'BO'			;
Magic string to detect our boot1f32 code


kGPTSignatureLow	EQU  'EFI '			;
GUID Partition Table Header Signature
kGPTSignatureHigh	EQU  'PART'
kGUIDLastDwordOffs	EQU  12				;
last 4 byte offset of a GUID

kPartCount			EQU  4				;
number of paritions per table
kPartTypeHFS		EQU  0xaf			;
HFS + Filesystem type
kPartTypePMBR		EQU  0xee			;
On all GUID Partition Table disks a Protective MBR (PMBR)
;
...
active flag enabled
kPartInactive	    EQU  0x00			;
active flag disabled
kHFSGUID	        EQU  0x48465300		;
first 4 bytes of Apple HFS Partition Type GUID.
kAppleGUID			EQU  0xACEC4365		;
last 4 bytes of Apple type GUIDs.
kEFISystemGUID		EQU  0x3BC93EC9		;
last 4 bytes of EFI System Partition Type GUID:
;
...
Assume we can deal with GPT but try to scan
    ;
...
testing partition
xor		dh, dh               					;
DH = 0 This will be used in Pass3 (partition is active, not HFS + ).

     cmp	    bh, kPass3								;
If this is Pass3 (BH = 1)
je	    .checkActive							;

check for active flag only.
...
Check for a HFS + partition.
jne     .continue

cmp	    bh, kPass2								;

It's HFS+. That's enough checking for Pass2,
je	    .tryToBoot								;

so try to boot (with DH = 1)
    ;
...
advance SI to next partition entry
loop    .loop                 		 	;
...
dec	    bl
jnz     .exit							;
didn't find Protective MBR before
call    checkGPT
...
check current GUID Partition for Apple's GUID type
je		.gpt_ok
...
or   dl, dl ;
to set flag Z = 0	;
dmazar's change to continue disk scanning if encountering invalid LBA.
...
AL = number of 512 - byte sectors to read.
     ;
ES:
BX = pointer to where the sectors should be stored.
     ;
DL = drive number (0x80 + unit number)
     ;
[my_lba] = starting LBA.
...
;
push    DWORD 0                 ;
offset 12, upper 32 - bit LBA
push    ds                      ;
For sake of saving memory,
    push    ds                      ;
push DS register, which is 0.
mov     ecx, [my_lba]           ;
offset 8, lower 32 - bit LBA
push    ecx
push    es                      ;
offset 6, memory segment
push    bx                      ;
offset 4, memory offset
xor     ah, ah                  ;
offset 3, must be 0
push    ax                      ;
...
DL    = drive number (80h + drive unit)
        ;
DS:
SI = pointer to Disk Address Packet
     ;
;
Returns:
;
...
;
ES:
DI   pointer to a NULL terminated string.
;
;
Clobber list:
;
...
BH = 0, BL = 1 (blue)
             cld                             ;
increment SI after each lodsb call
.loop:
lodsb                           ;
load a byte from DS:
SI into AL
...
Starting LBA for read_lba function
