...
In pass 1 we are walking on the standard path
;
by trying to hop on the active partition.
jne     .continue
xor	  	dh, dh               					;

Argument for loadBootSector to skip HFS + partition
;
...
In pass 2 we're going to find a HFS+ partition
...
cmp		eax, kEFISystemGUID		;

check current GUID Partition for EFI System Partition GUID type
jne		.gpt_continue
...
mov	    eax, [si + gpta.StartingLBA]			;
load boot sector from StartingLBA
mov	    [my_lba], eax
mov		dh, 1									;
...
no boot loader signature

mov	    si, kMBRPartTable						;
fake the current GUID Partition
mov	    [si + part.lba], eax					;

as MBR style partition for boot1h
...
pop     bx
ret												;
no more GUID partitions. Giving up.
