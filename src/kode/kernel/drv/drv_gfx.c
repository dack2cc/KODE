...
DRV_PRIVATE  void drv_gfx_DispData(DRV_GFX_SHEET_EXT * pstSheet_in, const DRV_GFX_DATA * pstData_in)
{
	...
	for (i = 0; i < pstData_in->h; ++i) {
		...
		for (j = 0; j < uiByte; ++j) {
			...
			if (0 != (uiData & 0x80)) {pbyBuf[0 + j * 8] = pstSheet_in->uiColor;}
			if (0 != (uiData & 0x40)) {pbyBuf[1 + j * 8] = pstSheet_in->uiColor;}
			if (0 != (uiData & 0x20)) {pbyBuf[2 + j * 8] = pstSheet_in->uiColor;}
			if (0 != (uiData & 0x10)) {pbyBuf[3 + j * 8] = pstSheet_in->uiColor;}
			if (0 != (uiData & 0x08)) {pbyBuf[4 + j * 8] = pstSheet_in->uiColor;}
			if (0 != (uiData & 0x04)) {pbyBuf[5 + j * 8] = pstSheet_in->uiColor;}
			if (0 != (uiData & 0x02)) {pbyBuf[6 + j * 8] = pstSheet_in->uiColor;}
			if (0 != (uiData & 0x01)) {pbyBuf[7 + j * 8] = pstSheet_in->uiColor;}
		}
	}
